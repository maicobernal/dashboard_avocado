...
def load_review(filename, engine):
    with pd.read_json(filename, lines=True, chunksize=200000) as reader:
    ...
    print('DONE UPLOADING TO MYSQL')


def load_tip(filename, engine):
        ...
        tip.to_sql('tip', con=engine, if_exists='append', index=False)
